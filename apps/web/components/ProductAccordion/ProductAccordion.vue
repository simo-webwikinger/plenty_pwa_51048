<template>
  <div data-testid="product-accordion">
    <UiAccordionItem
      ...
      summary-class="md:rounded-md w-full py-2 pl-4 pr-3 flex justify-between items-center select-none"
      v-model="productDetailsOpen"
    >
      <template #summary>
        <h2 class="font-bold font-headings text-lg leading-6 md:text-xl">
          {{ $t('productDetails') }}
        </h2>
      </template>
    ...
    <UiAccordionItem
      ...
      summary-class="md:rounded-md w-full hover:bg-white py-2 pl-4 pr-3 flex justify-between items-center select-none"
